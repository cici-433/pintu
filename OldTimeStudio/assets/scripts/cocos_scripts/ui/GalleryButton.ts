 ...
 * GalleryButton.ts
 *
 * 这是一个专门用于“相册”按钮的 UI 组件，严格对齐 Figma 节点 18:383 的结构与样式。
 *
...
export type GalleryButtonInitOptions = {
  /**
   * 按钮在父节点坐标系下的位置（本地坐标）。
   */
  position: Vec3;
...
export class GalleryButton extends Component {
   ...
   * 初始化/重建按钮 UI（可重复调用）。
   */
  public init(options: GalleryButtonInitOptions) {
    ...
    bgG.clear();
    bgG.fillColor = new Color(9, 122, 114, 255); // #097A72
    bgG.strokeColor = new Color(18, 74, 68, 255); // #124A44
